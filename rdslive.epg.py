...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('m3u_generator.log'),
...
class Config:
    ...
    CACHE_EXPIRY = int(os.getenv('CACHE_EXPIRY', 180))  # 3 minutes
    PORT = int(os.getenv('PORT', 5000))
    DEBUG = os.getenv('DEBUG', 'False').lower() == 'true'
    EPG_URL = os.getenv('EPG_URL', 'http://epg.iptv.ro/epg.xml')  # Default EPG source
    EPG_UPDATE_INTERVAL = int(os.getenv('EPG_UPDATE_INTERVAL', 3600))  # 1 hour
...
class RDSLiveM3UGenerator:
    def __init__(self, config: Config):
        ...
        self.video_cache = {}  # Simple cache for video URLs
        self.epg_cache = {}  # Cache for EPG data
        self.m3u_content = ""  # Cache for M3U content
        self.epg_content = ""  # Cache for EPG content
    ...
    def _parse_channel(self, post_id: str, article_class: str, article_content: str) -> Optional[Channel]:
        ...
        tvg_shift = "0"  # Default no shift
        tvg_chno = post_id  # Use post ID as channel number
    ...
    def get_video_source(self, post_id: str) -> Optional[str]:
        ...
        try:
            ...
            if json_response.get('success'):
                ...
                if raw_source_url:
                    # If the URL already contains a token (like canale-tv.com URLs), use it directly
                    if 'token=' in raw_source_url:
                        final_url = raw_source_url
                    else:
                        # Try to get the stream URL from the channel page directly
    ...
    def generate_m3u(self, channels: List[Channel]) -> str:
        ...
        # Process channels sequentially to avoid overwhelming the server
    ...
    def generate_epg(self, channels: List[Channel]) -> str:
        ...
        tv.set("generator-info-url", "https://github.com/yourusername/rds-live-m3u-generator")
        ...
        # Add program information (placeholder data)
        now = datetime.now()
        for channel in channels:
            # Generate some placeholder programs for the next 24 hours
            for i in range(6):  # 6 programs per channel
    ...
    def update_playlist(self) -> bool:
        ...
        try:
            ...
            if channels:
                ...
                # Generate and save M3U file with stream URLs
                m3u_content = self.generate_m3u(channels)
                self.m3u_content = m3u_content  # Cache the M3U content
                with open('rdslive_streams.m3u', 'w', encoding='utf-8') as f:
                    f.write(m3u_content)
                
                # Generate and save EPG file
                epg_content = self.generate_epg(channels)
                self.epg_content = epg_content  # Cache the EPG content
                with open('rdslive_epg.xml', 'w', encoding='utf-8') as f:
                    f.write(epg_content)
                
                # Count EPG programs
                self.stats.epg_programs = len(channels) * 6  # 6 programs per channel
    ...
    <script>
        ...
        function playChannel(channelId) {
            ...
            if (channel && channel.stream_url) {
                // Open the stream URL in a new window
...
<body>
    <div class="container">
        ...
        <div class="warning">
            <strong>⚠️ Updated with EPG Support</strong><br>
            This service now generates both M3U playlists and EPG (Electronic Program Guide) data.
        </div>
...
def download_playlist():
    ...
    if not os.path.exists('rdslive_streams.m3u'):
        generator.update_playlist()
    
    return send_file('rdslive_streams.m3u',
                    mimetype='audio/x-mpegurl', 
                    as_attachment=True,
...
def download_epg():
    ...
    if not os.path.exists('rdslive_epg.xml'):
        generator.update_playlist()
    
    return send_file('rdslive_epg.xml',
                    mimetype='application/xml', 
                    as_attachment=True,
